...
<template>
    <v-sheet class="room-card pa-3" :elevation="5" v-if="roomData">
        ...
        <v-row>
            <v-col>
                <p v-if="!roomPredictions" class="text-center" style="color: #89C4F4;"><small>Room occupancy predictions are currently unavailable.</small></p>
                <ForecastChart v-else :roomPredictions="roomPredictions" />
            </v-col>
        </v-row>
        <v-row>
            <v-col>
                <p v-if="roomPredictions?.sensorData.predictions"><small>Last predictions fetched: {{ formatDateString(roomPredictions.sensorData.predictions[0].timestamp) }}</small></p>  
